...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const fetchCheckoutData = async () => {
      ...
      try {
        ...
        // Fetch shop details
        if (currentShopId) {
          const shopResponse = await axios.get<Shop>(`http://localhost:8989/api/shops/${currentShopId}`);
        ...
        const productPromises = productIds.map(productId =>
          axios.get<BackendProduct>(`http://localhost:8989/api/products/${productId}`)
        );
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8989/api/orders", orderPayload);
