...
const VendorManagement = () => {
  ...
  const addShopAndVendorMutation = useMutation({
    ...
    onSuccess: (response) => {
        ...
        setAddFormData({
            name: '', address: '', contactNumber: '', cuisine: 'Healthy Food', active: true,
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async (vendorId: number) => {
      ...
      const payload: UpdateVendorRequest = {
        ...
        active: !vendorToToggle.active, // Toggle vendor's active status
  ...
  const handleUpdateVendor = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedVendor) {
        // Ensure the ID is correctly set in editFormData
        const payload: UpdateVendorRequest = {
            ...editFormData,
            id: selectedVendor.id, // Ensure the correct ID is used
        };
